...
def venues():
    ...
    for venue in venues:
        ...
        if venue.city not in locations:
...
def create_venue_submission():
    try:
        ...
        db.session.rollback()
        flash(
          "An error occurred. Venue " + data["name"] + " could not be listed."
        )
    finally:
        db.session.close()
    return render_template("pages/home.html")
...
def edit_artist_submission(artist_id):
    ...
    try:
        ...
        db.session.commit()
        flash(
          "The Artist " + request.form["name"]
          + " has been successfully updated!"
        )
    except Exception as e:
        db.session.rollback()
        flash(
          "The Artist " + request.form["name"] + " could not be updated!"
        )
    finally:
        db.session.close()
...
def edit_venue_submission(venue_id):
    ...
    try:
        ...
        db.session.commit()
        flash(
          "The Venue " + request.form["name"] +
          " has been successfully updated!"
        )
    except Exception as e:
        db.session.rollback()
        flash(
          "The Venue " + request.form["name"] + " could not be updated!"
        )
    finally:
        db.session.close()
...
def create_artist_submission():
    try:
        ...
        db.session.rollback()
        flash(
          "An error occurred. Artist " + data["name"] + " could not be listed."
        )
    finally:
        db.session.close()
